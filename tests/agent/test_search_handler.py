import unittest
from src.agent.search_handler import catalog_search_tool # <--- CAMBIO AQUÍ
...
class TestSearchHandler(unittest.TestCase):
    ...
    def test_search_no_criteria(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products})
        self.assertEqual(len(results), 5) # Devuelve todos los productos

    def test_search_by_query_name(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "query": "Smartphone"})
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], "MP001")

    def test_search_by_query_description(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "query": "Sonido increíble"})
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], "MP002")

    def test_search_by_query_tags(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "query": "cocina"})
    ...
    def test_search_by_category(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "category": "Hogar"})
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], "MP003")

    def test_search_by_brand(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "brand": "TechGlobal"})
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], "MP001")

    def test_search_by_min_price(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "min_price": 300.00})
    ...
    def test_search_by_max_price(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "max_price": 150.00})
    ...
    def test_search_by_price_range(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "min_price": 100.00, "max_price": 500.00})
    ...
    def test_search_by_min_rating(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "min_rating": 4.7})
    ...
    def test_search_by_in_stock_true(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "in_stock": True})
    ...
    def test_search_by_in_stock_false(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "in_stock": False})
        self.assertEqual(len(results), 1)
    ...
    def test_search_combined_criteria(self):
        ...
        results = catalog_search_tool.invoke({
            "marketplace_products": self.mock_products,
    ...
    def test_search_no_results(self):
        results = catalog_search_tool.invoke({"marketplace_products": self.mock_products, "query": "inexistente", "brand": "NoBrand"})
        self.assertEqual(len(results), 0)

    def test_search_empty_product_list(self):
        results = catalog_search_tool.invoke({"marketplace_products": [], "query": "smartphone"})
        self.assertEqual(len(results), 0)
    ...
    def test_search_product_without_optional_fields(self):
        ...
        results_query = catalog_search_tool.invoke({"marketplace_products": product_sin_campos, "query": "Pelado"})
        self.assertEqual(len(results_query), 1)

        results_price = catalog_search_tool.invoke({"marketplace_products": product_sin_campos, "max_price": 100})
        self.assertEqual(len(results_price), 0) # No debería fallar, pero no machea

        results_stock = catalog_search_tool.invoke({"marketplace_products": product_sin_campos, "in_stock": True})
